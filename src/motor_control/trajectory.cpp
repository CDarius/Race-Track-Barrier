...
/**
Sum the integet part and the decimal part of encoder counts

:param count: Encoder counts integer part in count
:param count_ext: Encoder counts decimal part in millicounts
:return: Sum of integer and decimal encoder counts (millicounts)
 */
...
/**
Split encoder counts in millicounts into integer and decimal part

:param mcount: Encoder count in millicounts
:return: A tuple with the following informations: Encoder count integer part (count), encoder count decimal part (millicount)
*/
...
/**
    Multiply speed by time to get a distance

    :param b: Speed in enc count/sec
    :param t: Time in us
    :return: Encoder counts in millicounts
 */
...
/**
    Calculate a distance from acceleration and time

    :param b: Acceleration in enc count/sec^2
    :param t: Time in us
    :return: Encoder counts in millicounts
 */
...
/**
Make a stationary (no movement) trajectory

:param t0: Start time (us)
:param th0: Encoder count at start of maneuver (integer part)
*/
...
/**
Make a time based ramp. Accelerated to target speed and keep running

:param t0: Start time (us)
:param duration: Maneuver duration in us. User Trajectory.DURATION_FOREVER for infinite duration
:param th0: Encoder count at start of maneuver (integer part)
:param th0_ext: Encoder count at start of maneuver (decimal part only) in millicounts
:param w0: Initial speed in enc count/sec
:param wt: Target speed in enc count/sec
:param wmax: Max speed in enc count/sec
:param a: Acceleraton in enc count/sec^2
:param amax: Max acceleration in enc count/sec^2
 */
pbio_error_t pbio_trajectory_make_time_based(pbio_trajectory_t *ref, int32_t t0, int32_t duration, int32_t th0, int32_t th0_ext, int32_t w0, int32_t wt, int32_t wmax, int32_t a, int32_t amax) {
    ...
    int32_t abs_max = PIO_MIN(wmax, max_init);
    w0 = PIO_MAX(-abs_max, PIO_MIN(w0, abs_max));
    wt = PIO_MAX(-abs_max, PIO_MIN(wt, abs_max));
...
/**
Make an angle base trajectory from angle th0 to angle th3 with target speed w

:param t0: Start time (us)
:param th0: Encoder count at start of maneuver (integer part)
:param th3: Encoder count at end of maneuver (integer part)
:param w0: Initial speed in enc count/sec
:param wt: Target speed in enc count/sec
:param wmax: Max speed in enc count/sec
:param a: Acceleraton in enc count/sec^2
...
pbio_error_t pbio_trajectory_make_angle_based(pbio_trajectory_t *ref, int32_t t0, int32_t th0, int32_t th3, int32_t w0, int32_t wt, int32_t wmax, int32_t a, int32_t amax) {
    ...
    // Limit initial speed
    w0 = PIO_MAX(-wmax, PIO_MIN(w0, wmax));
...
/**
Return the status of a trajectory (position, speed, acceleration) at the given time

:param time_ref: Time instant where calculate the trajectory status (us)
:param count_ref: Return position integer part (count), 
:param count_ref_ext: Return position decimal part (millicount)
:param rate_ref: Return speed (enc count/s)
:param acceleration_ref: Return acceleration (enc count/sec^2)
*/
